...
const run = async () => {
    try {
        ...
        await db.execute(sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS registered_device_id text`);

        console.log('Checking/Adding device_last_seen to users...');
        await db.execute(sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS device_last_seen timestamp`);
        ...
        try {
            ...
            await db.execute(sql`DROP TABLE IF EXISTS qr_validation_logs CASCADE`);
